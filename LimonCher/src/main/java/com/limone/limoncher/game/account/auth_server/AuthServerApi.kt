...
package com.limone.limoncher.game.account.auth_server
...
import com.limone.limoncher.game.account.auth_server.models.AuthRequest
import com.limone.limoncher.game.account.auth_server.models.AuthResult
import com.limone.limoncher.game.account.auth_server.models.Refresh
...
import com.limone.limoncher.utils.network.safeBodyAsText
import com.limone.limoncher.utils.string.decodeUnicode
...
class AuthServerApi(private var baseUrl: String) {
    ...
    private val isElyBy = baseUrl.trimEnd('/').equals("https://authserver.ely.by", ignoreCase = true)

    init {
        baseUrl = formatUrl(baseUrl)
    }

    private fun authPath(path: String): String =
        if (isElyBy) path.replaceFirst("/authserver/", "/auth/") else path
    ...
    suspend fun authenticate(context: Context, userName: String, password: String): AuthResult {
        ...
        return requestAuth(Gson().toJson(authRequest), authPath("/authserver/authenticate"))
    }
    ...
    suspend fun refreshToken(context: Context, account: Account, select: Boolean): AuthResult {
        ...
        return requestAuth(Gson().toJson(refresh), authPath("/authserver/refresh"))
    }
    ...
    suspend fun validate(context: Context, account: Account): Boolean {
        ...
        return withContext(Dispatchers.IO) {
            try {
                val response = GLOBAL_CLIENT.post("$baseUrl${authPath("/authserver/validate")}") {
