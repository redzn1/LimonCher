...
import com.limone.limoncher.database.AppDatabase
import com.limone.limoncher.game.account.auth_server.data.AuthServer
import com.limone.limoncher.game.account.auth_server.data.AuthServerDao
...
import com.limone.limoncher.utils.network.isNetworkAvailable
...
private const val ELY_BY_BASE_URL = "https://authserver.ely.by"
private const val ELY_BY_REGISTER_URL = "https://account.ely.by/register"
...
object AccountsManager {
    ...
    fun initialize(context: Context) {
        ...
        // Keep Ely.by available out of the box while preserving the existing
        // custom authentication-server system. The server is stored once in
        // the same Room table used for other Yggdrasil-compatible providers.
        scope.launch {
            runCatching {
                if (authServerDao.getServer(ELY_BY_BASE_URL) == null) {
                    authServerDao.saveServer(
                        AuthServer(
                            baseUrl = ELY_BY_BASE_URL,
                            serverName = "Ely.by",
                            register = ELY_BY_REGISTER_URL
                        )
                    )
                }
                reloadAuthServers()
            }.onFailure {
                Logger.warning(TAG, "Unable to initialize default Ely.by auth server", it)
            }
        }
    }
    ...
    /**
     * Refresh the current account state.
     */
    ...
    private fun checkLimit(): Boolean {
        // LimonCher intentionally supports offline and alternative Yggdrasil
        // accounts without a region gate. Authentication providers still
        // validate credentials through their own servers.
        return false
    }
