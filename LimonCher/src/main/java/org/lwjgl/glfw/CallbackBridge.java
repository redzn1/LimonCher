...
import static com.limone.limoncher.bridge.ZLBridgeStatesKt.CURSOR_DISABLED;
import static com.limone.limoncher.bridge.ZLBridgeStatesKt.CURSOR_ENABLED;
...
public class CallbackBridge {
    ...
    public static boolean notifyLauncher(int type, int... action) {
        if (action == null || action.length == 0) {
            LoggerBridge.append("LimonCher: SDL notification has no action");
            return false;
        }
        switch (type) {
            case NOTIF_TYPE_SDL:
                if (action[0] == ACTION_INIT_LAUNCHER_INTEGRATION) {
                    ...
                    try {
                        LoggerBridge.append("LimonCher: loading SDL3");
                        System.loadLibrary("SDL3");
                        LoggerBridge.append("LimonCher: loading SDL2");
                        System.loadLibrary("SDL2");
                        LoggerBridge.append("LimonCher: setting up SDL JNI");
                        SdlBridge.setupJNI();
                        LoggerBridge.append("LimonCher: binding SDL surface");
                        ...
                        LoggerBridge.append("LimonCher: SDL support enabled!");
                        return true;
                    } catch (Throwable e) {
                        ...
                        LoggerBridge.append("LimonCher: SDL launcher integration is unavailable:\n" + trace);
                    }
                }
    ...
    private static void onDirectInputEnable() {
        LoggerBridge.append("LimonCher: Direct gamepad input enabled");
